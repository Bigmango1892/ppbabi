...
def log_sum_exp(vec):
    max_score = vec[range(BATCH_SIZE), argmax(vec)]
    max_score_broadcast = max_score.view(-1, 1).expand(BATCH_SIZE, vec.size()[1])
...
class BiLSTM_CRF(nn.Module):
    ...
    def _feat_splice(self, sentence, lstm_feats, embeds):
        ...
        eig = torch.cat((lstm_feats_split, embeds, sentence_pos.view(-1, len(sentence), 1), sentence_con.view(-1, len(sentence), 1), sentence_seg.view(-1,len(sentence), 1)),2)
    ...
    def _forward_alg(self, feats):
        ...
        for feat in feats:
            ...
            for next_tag in range(self.tagset_size):
                # broadcast the emission score: it is the same regardless of the previous tag
                emit_score = feat[:, next_tag].view(-1, 1).expand(BATCH_SIZE, self.tagset_size)
                # the i-th entry of trans_score is the score of transitioning to next_tag from i
    ...
    def _score_sentence(self, feats, tags):
        ...
        tags = [torch.cat([torch.tensor([self.tag_to_ix[START_TAG]], dtype=torch.long), tag,
                           self.tag_to_ix['O'] * torch.ones(feats.size()[0]-tag.size()[0])]).view(1, -1) for tag in tags]
        tags = torch.cat(tags).long()
        for i, feat in enumerate(feats):
            score = score + self.transitions[tags[:, i + 1], tags[:, i]] + feat[range(BATCH_SIZE), tags[:, i + 1]]
        score = score + self.transitions[self.tag_to_ix[STOP_TAG], tags[:, -1]]
        return score
    ...
    for epoch in range(1):
        ...
        if not FLAG:
            for i in range(len(train_data)):
                model.zero_grad()
                tag_to_ix_new = {"B-AbilityTag": 0, "I-AbilityTag": 1, "O": 2, START_TAG: 3, STOP_TAG: 4, 'B-LevelTag': 2,
                             'I-LevelTag': 2}
            ...
            for step, batch in enumerate(train_data_loader):
                ...
                batch_targets = [torch.tensor([tag_to_ix_new[t] for t in tags], dtype=torch.long) for tags in batch_tags]

                # Step 3. Run our forward pass.
                loss = model.loss_function(batch_feature, batch_targets)
                print(loss)
